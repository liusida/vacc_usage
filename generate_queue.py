...
Compute the current usage and queue by reading the data file generated by command `sinfo` and `squeue`.
Generate a plot into a html file `queue.plots.html`.
"""
...
def get_total_resources(cluster='bluemoon'):
    """
    Read `sinfo` data file, and get the total amount of assets.
    """
...
"""
For each cluster, query the total amount of resources VACC has.
"""
...
def get_resources_amount(cluster='bluemoon', status='running'):
    """
    Read `squeue` data file, and get the current usage or the queue.
    """
    c = cluster_settings[cluster]
...
"""
For each cluster, query the queue and current usage:
"""
for key, value in cluster_settings.items():
